...
extern "C"
{
#include <TDE/lua/lua.h>
#include <TDE/lua/lauxlib.h>
#include <TDE/lua/lualib.h>
}
...
#define LUA_OPEN_USER_LIB( spaceName, lib ) \
	TDE::CLuaRegistLibProxy s_LuaRegistLibProxy_##lib(spaceName, (luaL_reg*)lib);
...
#define LUA_DECL_WRAPPER( type_c, type_wrapper ) \
	typedef TDE::luaUserdataWrapper<type_c> type_wrapper;

	class TDE_IMPORT LuaEx
	{
	public:
		/** make do file. */
	...
	public:
		...
		///	获取包含在reg库中所有变量名称
		static std::vector<CString> GetFunctionName( const luaL_reg* reg );
	...
	typedef TDE::CDelegate( void(lua_State*) )  LuaRegistLibFunc;

	class TDE_IMPORT CLuaCLibRegister
	{
	public:
	...
	class TDE_IMPORT CLuaRegistLibProxy
	{
	public:
